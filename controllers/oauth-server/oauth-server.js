...
function coreOAuth(app) {
    ...
    .get(function (req, res) {
        ...
        function qBy (ft) {
            if (ft === 'id') {
                return oauth2.findClient({id : req.params.clientId});
            }
            if (ft === 'email') {
                return oauth2.findClient({email : req.params.clientId});
            }
            if (ft === 'device') {
                return oauth2.findClient({device : req.params.clientId});
            }
    ...
    .delete(function (req, res, next) {
        ...
        function qBy (ft) {
            if (ft === 'id') {
                return oauth2.removeAClient({id : req.params.clientId});
            }
            if (ft === 'email') {
                return oauth2.removeAClient({email : req.params.clientId});
            }
            if (ft === 'device') {
                return oauth2.removeAClient({device : req.params.clientId});
            }
    ...
    .get(function (req, res, next){
        if (req.headers)
        next();
    }, OAuth2Middleware.authorization, OAuth2Middleware.dialog);
    // app.post(OAuth2Middleware.decision);
